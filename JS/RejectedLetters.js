...
function btnSearch_Click() {
    ...
    if (AllDocuments == null) {
        ...
        var documentList = GetRejectedDocuments();
...
function GetRejectedDocuments() {
    try {
        var url = _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('" + ListNames.PROPERTYLETTERS + "')/Items?$select=*,Author/Title,File&$filter=Approval_x0020_Status eq 'Rejected'";
...
function BindgrvDocSelection(documents) {
    tblGenLett = $('#grvRejectedDoc').DataTable({
        "destroy": true,
        "data": documents,
        "columns": [
            { "data": "File.Name" },
